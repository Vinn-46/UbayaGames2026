...
@section('content')
        ...
        <section class="w-full px-4 sm:px-6 mb-32 relative z-10 grow flex flex-col items-center">
            <div class="grid grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8 w-full max-w-4xl shrink-0">
                @foreach ($houses as $house)
                    <div onclick="openModal('{{ $house['name'] }}', '{{ $house['facultyName'] }}', '{{ asset('assets/fakultas/' . $house['image']) }}', '{{ $house['desc'] }}', '{{ $house['line_id'] }}')"
                        ...
                        <div class="relative w-full flex-1 mb-4">
                            <img src="{{ asset('assets/fakultas/' . $house['image']) }}" alt="{{ $house['name'] }}"
                                class="absolute inset-0 w-full h-full object-contain object-center group-hover:scale-110 transition duration-300">
                        </div>
                        <h3
                            class="w-full text-white text-center text-[10px] sm:text-xs font-semibold tracking-wider uppercase transition shrink-0">
                            {{ $house['name'] }}
                        </h3>
                    </div>
                @endforeach
    ...
    <div id="houseModal" class="fixed inset-0 z-[100] hidden items-center justify-center p-4">
        ...
        <div class="relative bg-gray-900 border border-white/20 rounded-2xl w-full max-w-lg p-8 flex flex-col items-center text-center transform scale-95 opacity-0 transition-all duration-300"
            id="modalContent">
            ...
            <h4 id="fakultas" class="text-m font-bold text-yellow-500 uppercase tracking-widest mb-4">Nama Fakultas
            </h4>
    ...
    <script>
        ...
        function openModal(name, facultyName, imageSrc, desc, cp, lineId) {
            modalTitle.innerText = name;
            fakultas.innerText = facultyName;
